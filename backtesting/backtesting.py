...
Versión: Datos separados H4 y M15 con columna time
"""
...
class Config:
    DATASET_H4_PATH = 'XAUUSD_4H.csv'
    DATASET_M15_PATH = 'XAUUSD_15M.csv'
    COL_OPEN = 'open'
    COL_HIGH = 'high'
    COL_LOW = 'low'
    COL_CLOSE = 'close'
    ...
    VOLUME_MULTIPLIER = 0
    HORAS_EVITAR = [0, 3, 7, 11, 19, 22, 23]
    #profit: 1038% //0, 3, 7, 11, 19, 22, 23
    ...
    PLOT_RESULTS = True
    SAVE_REPORT = True
    REPORT_FILENAME = 'XAUUSD_backtest_95k_results.html'
    ...
    def load_csv_file(self, filepath):
        try:
            ...
            if 'time' in df.columns:
                df['time'] = pd.to_datetime(df['time'], format=self.config.DATETIME_FORMAT)
            else:
                print(f"❌ Error: No se encontró columna 'time' en {filepath}")
                return None
